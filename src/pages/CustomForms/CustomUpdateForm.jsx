...
//   // ✅ Don't fetch if suggestions were manually closed
//   if (!showSuggestions && suggestion.length > 0) {
//     return;
//   }

//   // ✅ NEW: Don't fetch if we already have this exact search
//   if (suggestion.length > 0 && suggestion.some(s => s.us_id === searchId)) {
//     return;
//   }
...
const CustomUpdateForm = () => {
  ...
  // ✅ Add ref to track suggestion selection
  ...
  // ✅ Debounced suggestion search with bug fix
  useEffect(() => {
    if (!searchId.trim()) {
      setSuggestion([]);
      setShowSuggestions(false);
    ...
    // ✅ Skip if suggestions are manually closed
    if (!showSuggestions) {
      return;
    }
  ...
  }, [owner_id, currentTable, visibleColumns, columns]);
  ...
  const handleSuggestionClick = useCallback((value, tableName) => {
    justSelectedSuggestion.current = true; // ✅ Set flag to prevent API call
    setSearchId(value);
    setCurrentTable(tableName);
    setSuggestion([]);
    setShowSuggestions(false);
  ...
  const handleSuggestionInput = useCallback((e) => {
    justSelectedSuggestion.current = false; // ✅ Reset flag when user types
    setSearchId(e.target.value);
    setShowSuggestions(true); // ✅ Re-enable suggestions
    if (error) setError(null);
  }, [error]);
