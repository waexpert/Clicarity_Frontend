...
// Working RecordFormModal with status set to the first process Name
import React, { useEffect, useMemo, useState } from 'react';
import {
  Dialog,
  DialogContent,
...
const RecordFormModal = ({
  ...
  const [isDetailSubmitting, setIsDetailSubmitting] = useState(false);
  const [isMainSubmitting, setIsMainSubmitting] = useState(false);
  const [processSteps, setProcessSteps] = useState([]); // NEW: Store process_steps
  ...
  // NEW: Effect to set default status value when form opens
  useEffect(() => {
    if (isOpen && mode === 'create' && columns.length > 0) {
      // Check if there's a status column
      const hasStatusColumn = columns.some(col => col.id === 'status' || col.id.toLowerCase() === 'status');
      
      if (hasStatusColumn && processSteps.length > 0) {
        // Only set if status is not already set
        if (!formData.status || formData.status === '') {
          onFieldChange('status', processSteps[0]);
          console.log('Setting default status to:', processSteps[0]);
        }
      }
    }
  }, [isOpen, mode, columns, processSteps, formData.status]);
  ...
  const fetchTableStructure = async (tableName) => {
    try {
      ...
      let dropdownSetup = {};
      let columnOrder = {};
      let fetchedProcessSteps = []; // NEW: Extract process_steps

      if (dropdownResponse.data.exists && dropdownResponse.data.setup) {
        dropdownSetup = dropdownResponse.data.setup.mapping || {};
        columnOrder = dropdownResponse.data.setup.columnOrder || {};
        fetchedProcessSteps = dropdownResponse.data.setup.process_steps || []; // NEW: Get process_steps
        
        // NEW: Store process_steps in state
        setProcessSteps(fetchedProcessSteps);
        console.log('Fetched process_steps:', fetchedProcessSteps);
      }
      ...
      return {
        ...
        columnOrder: columnOrder,
        processName: null,
        processSteps: fetchedProcessSteps // NEW: Return process_steps
      };
    } catch (error) {
      console.error('Error fetching table structure:', error);
      throw error;
  ...
  */
  ...
  const handleTableSelection = async () => {
    ...
    try {
      ...
      // NEW: Set default status for detail form if it has status column and process_steps
      const hasStatusColumn = tableData.columns.some(col => col.id === 'status' || col.id.toLowerCase() === 'status');
      if (hasStatusColumn && tableData.processSteps && tableData.processSteps.length > 0) {
        initialDetailData.status = tableData.processSteps[0];
        console.log('Setting default status for detail form:', tableData.processSteps[0]);
      }
  ...
  const handleCancel = () => {
    ...
    setDetailProcessName(null);
    setCurrentParentUsId(null);
    setProcessSteps([]); // NEW: Reset process_steps
    onClose();
  };
