...
// function FormPreview({ fields, onSubmit, formId ,tableName,submit,recordId}) {
...
function FormPreview({ fields, onSubmit, formId, tableName, submit, recordId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitted(true); // ✅ trigger success screen
  ...
  // ✅ Success screen — taken from StatusUpdate
  if (submitted) {
    return (
      <div style={successStyles.container}>
        <div style={successStyles.iconWrapper}>
          <span style={successStyles.icon}>✓</span>
        </div>
        <h2 style={successStyles.heading}>Success!</h2>
...
// ✅ Success screen styles matching StatusUpdate
const successStyles = {
  container: {
    display: 'flex',
...
function renderClicaryField(field, formData, handleChange) {
  ...
  switch (field.type) {
    case 'heading':
      return (
        <h1 style={{ margin: 0, fontSize: '28px', fontWeight: '700', color: '#1F2937', lineHeight: 1 }}>
          {field.content}
        </h1>
      );
    ...
    case 'subheading':
      return (
        <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '600', color: '#374151', lineHeight: 1.2 }}>
          {field.content}
        </h2>
      );
    ...
    case 'body':
      return (
        <p style={{ margin: 0, lineHeight: 1.6, fontSize: '14px', color: '#6B7280' }}>
          {field.content}
        </p>
      );
    ...
    case 'caption':
      return (
        <small style={{ color: '#9CA3AF', fontSize: '13px', fontStyle: 'italic' }}>
          {field.content}
        </small>
      );
    ...
    case 'image':
      return (
        <div style={{
          width: '100%', height: 200, background: '#F3F4F6', borderRadius: 6,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: '#9CA3AF', border: '2px dashed #D1D5DB'
        }}>
          Image Placeholder
        </div>
      );
    ...
    case 'textarea':
      return (
        <div>
          <label style={labelStyle}>{field.label}{field.required && <span style={requiredStar}>*</span>}</label>
          <textarea
    ...
    case 'radio':
      return (
        <div>
          ...
          <div style={{ marginTop: '8px' }}>
            {field.options.map((o, i) => (
              <label key={i} style={{ display: 'flex', alignItems: 'center', marginBottom: '10px', cursor: 'pointer', fontSize: '14px', color: '#374151' }}>
                <input type="radio" name={field.name} value={o} required={field.required}
                  checked={value === o} onChange={(e) => handleChange(field.name, e.target.value)}
                  style={{ marginRight: '10px', width: '16px', height: '16px', cursor: 'pointer', accentColor: '#5B9BD5' }}
                />
                {o}
              </label>
            ))}
    ...
    case 'checkbox':
      return (
        <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', fontSize: '14px', color: '#374151' }}>
          <input type="checkbox"
            style={{ marginRight: '10px', width: '18px', height: '18px', cursor: 'pointer', accentColor: '#5B9BD5' }}
            required={field.required} checked={value === true}
            onChange={(e) => handleChange(field.name, e.target.checked)}
          />
          <span>{field.label}{field.required && <span style={requiredStar}>*</span>}</span>
        </label>
      );

    case 'optin':
      return (
        <label style={{ display: 'flex', alignItems: 'flex-start', cursor: 'pointer', fontSize: '14px', color: '#374151', lineHeight: 1.5 }}>
          <input type="checkbox"
            style={{ marginRight: '10px', marginTop: '2px', width: '18px', height: '18px', cursor: 'pointer', accentColor: '#5B9BD5', flexShrink: 0 }}
            required={field.required} checked={value === true}
            onChange={(e) => handleChange(field.name, e.target.checked)}
          />
          <span>{field.label}{field.required && <span style={requiredStar}>*</span>}</span>
    ...
    case 'passcode':
      return (
        <div>
          <label style={labelStyle}>{field.label}{field.required && <span style={requiredStar}>*</span>}</label>
          <input type="text" maxLength={field.maxLength} style={inputStyle}
            placeholder={field.placeholder} required={field.required} value={value}
            onChange={(e) => handleChange(field.name, e.target.value)}
            onFocus={(e) => e.target.style.borderColor = '#5B9BD5'}
            onBlur={(e) => e.target.style.borderColor = '#D1D5DB'}
          />
